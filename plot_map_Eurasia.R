...
metrics_joined <- metrics_joined %>% 
  ...
  filter(Macroarea == "Africa" | Macroarea == "Papunesia" | Macroarea == "Australia" | Macroarea == "Eurasia") %>%
...
lakes <- map_data("lakes", col="white", border="gray", margin=T, ylim=c(-45,80), xlim=c(-30, 180), wrap=c(-25,335) )
...
basemap <- ggplot(combination) +
  ...
  coord_map(projection = "vandergrinten", ylim=c(-45,80), xlim=c(-30, 180))
...
i_labelled <- basemap + geom_point(data=combination_shaped_0, aes(x=Longitude, y=Latitude, color=informativity_st, fill=informativity_st), size=1, pch=21,# stroke=NA,
                                   position = position_jitter(width = 1, height = 1, seed=123)) + 
  geom_point(data=combination_shaped_1, aes(x=Longitude, y=Latitude, fill=informativity_st), pch=21, size=2.5, colour = "black",   
             position = position_jitter(width = 1, height = 1, seed=123)) +
  ...
  scale_color_viridis_c(option="viridis", guide="none", direction=-1) +
  scale_shape_manual(values = c(21, 24)) +
  theme(text = element_text(size = 20), legend.key.size = unit(0.5, units="cm"), legend.position="right") + 
  guides(fill = guide_colourbar()) +
  labs(fill="informativity") +
  geom_text_repel(aes(x=Longitude, y=Latitude, label=Name), #size=5,
                  data  = subset(combination, (Family_ID == "ural1272" & informativity_st > -0.5)), 
                  size          = 5,
                  box.padding   = 0.7,
                  max.overlaps = Inf,
                  point.padding = 0.1)
...
ggsave(file="output/map_informativity_labelled.svg", plot=i_labelled, width=10, height=9)
