...
effs_I <- read.csv("output_tables_reduced/ effects Informativity_social_models .csv")
effs_I$variable <- "informativity"

effs_B <- read.csv("output_tables_reduced/ effects Boundness_social_models .csv")
...
effs_I <- read.csv("output_tables_reduced/ effects Informativity_social_only_models .csv")
effs_I$variable <- "informativity"

effs_B <- read.csv("output_tables_reduced/ effects Boundness_social_only_models .csv")
effs_B$variable <- "fusion"
...
eff_main_plot_df = effs_main %>%
  ...
  mutate(effect = factor(effect, levels=c("Education", "Neighbours", "Official status", "L2 (combined)", "L1 (combined)", "L2", "L1"))) %>%
...
effs_main_plot_bw
ggsave(file="output_reduced/effects_plot.svg", plot=effs_main_plot_bw, height = 22, width = 36)
