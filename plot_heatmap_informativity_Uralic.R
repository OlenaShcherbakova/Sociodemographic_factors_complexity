...
uralic_fam_tree <- ggtree(tree, layout = 'rect', branch.length='none') 
uralic_fam_tree <- uralic_fam_tree %<+% metrics_joined + geom_tiplab(aes(label=Name), size=5)

p1 <- gheatmap(uralic_fam_tree, df1, offset=6, width=.2, colnames_angle=0, colnames_position = "top", colnames=F, #removing column names
               font.size=5, hjust=0.5, color = FALSE) + #ylim(-5, 27) + #ylim(-5, 1450)
  scale_fill_viridis_c(option="viridis", direction=-1, limits=c(min, max), guide="none") + 
  theme(legend.position = c(1, 1)) + theme(legend.position = "right",  legend.box = "vertical", text = element_text(size = 50), legend.key.size = unit(0.9, 'cm')) 

svg("output/heatmap_Uralic_informativity.svg", width=7, height=7)
p1
dev.off() 

triple_plot <- b_labelled / i_labelled | p1 #+ plot_annotation(tag_levels = '1')

svg("output/triple_plot.svg", width=12, height=7)
triple_plot
dev.off() 
