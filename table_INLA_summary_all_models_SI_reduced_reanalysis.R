#generate summary tables of WAIC values and effects for boundness and informativity models with "reduced" sample setting
#(excluding social-only models and models including non-linear effects)
...
effs_B_SP <-
  ...
  rename(lower = 2,
         ...
         mean = 3)
effs_B_social <-
  read.csv("output_tables_reanalysis_reduced/ effects Boundness_reanalysis_social_models .csv") %>%
  rename(lower = 2,
         upper = 4,
         mean = 3)
effs_B_social_only <-
  read.csv("output_tables_reanalysis_reduced/ effects Boundness_reanalysis_social_only_models .csv") %>%
  rename(lower = 2,
         upper = 4,
         mean = 3)
...
effs_B_SP_L2 <-
  ...
  rename(lower = 2,
         ...
         mean = 3)
effs_B_social_L2 <-
  read.csv("output_tables_reanalysis_reduced/ effects Boundness_L2_social_models .csv") %>%
  rename(lower = 2,
         upper = 4,
         mean = 3)
effs_B_social_only_L2 <-
  read.csv("output_tables_reanalysis_reduced/ effects Boundness_L2_social_only_models .csv") %>%
  rename(lower = 2,
         upper = 4,
         mean = 3)
...
effs_I_SP_L2 <-
  ...
  rename(lower = 2,
         ...
         mean = 3)
effs_I_social_L2 <-
  read.csv("output_tables_reanalysis_reduced/ effects Informativity_L2_social_models .csv") %>%
  rename(lower = 2,
         upper = 4,
         mean = 3)
effs_I_social_only_L2 <-
  read.csv("output_tables_reanalysis_reduced/ effects Informativity_L2_social_only_models .csv") %>%
  rename(lower = 2,
         upper = 4,
         mean = 3)
...
write.csv(all_effs, "output_tables_reanalysis_reduced/Table_INLA_all_models.csv")
...
save_as_docx(
  ...
  path = "output_tables_reanalysis_reduced/Table_INLA_all_models.docx")
