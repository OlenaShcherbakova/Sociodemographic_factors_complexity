...
GB_wide <- read_tsv(file.path("data", "GB_wide", "GB_wide_strict.tsv"), show_col_types = F)	%>% 
  filter(na_prop <= 0.25 ) %>% 
  dplyr::select(-na_prop)

#read in sheet with scores for whether a feature denotes informativity
GB_informativity_points <- read_csv(file.path("data", "GB_wide", "parameters.csv"), show_col_types = F) %>%
