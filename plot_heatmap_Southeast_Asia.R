...
metrics_joined <- metrics_joined %>% mutate(Language_ID_2 = Language_ID) 

metrics_joined <- metrics_joined %>% column_to_rownames(var = "Language_ID_2") 

df1 <- metrics_joined %>% filter(AUTOTYP_area == "Southeast Asia") %>% dplyr::select(boundness_st) %>% rename(boundness=boundness_st)
df2 <- metrics_joined %>% filter(AUTOTYP_area == "Southeast Asia") %>% dplyr::select(informativity_st) %>% rename(informativity = informativity_st)
...
coloured_branches <- ggtree(tree, layout = 'rect', branch.length='none', size=0.5) + geom_tiplab(size=2)
...
p1 <- gheatmap(coloured_branches, df1, offset=1, width=.1, colnames_angle=0, 
               ...
               font.size=20, hjust=0.5, color = NULL) + #ylim(-5, 1480) + #ylim(-5, 1450)
  scale_fill_viridis_c(option="magma", direction=-1) + labs(fill="fusion") + theme(legend.box = "horizontal", legend.position = "bottom", text = element_text(size = 30), legend.key.size = unit(1.6, 'cm')) +
  #xlim(-1, 55) +
  geom_cladelabel(node=nodes[1], label=biggest_families_verbose[1], offset=3, align=TRUE, fontsize=11) +
  geom_cladelabel(node=nodes[2], label=biggest_families_verbose[2], offset=3, align=TRUE, fontsize=11) +
  geom_cladelabel(node=nodes[3], label=biggest_families_verbose[3], offset=3, align=TRUE, fontsize=11) +
  geom_cladelabel(node=nodes[4], label=biggest_families_verbose[4], offset=3, align=TRUE, fontsize=11) +
  geom_cladelabel(node=nodes[5], label=biggest_families_verbose[5], offset=3, align=TRUE, fontsize=11) + 
  labs(fill="fusion")
...
ggsave(file="output/plot_toy_heatmap.svg", plot=p1, width=20, height=30)
ggsave(file="output/plot_toy_heatmap.pdf", plot=p1, width=20, height=30)
